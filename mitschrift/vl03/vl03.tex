...
\subsubsection*{Bellmann's Algorithm}
\begin{enumerate}
\item $D(i) = \{ \infty: i<n and NEXT(i) = NIL, 0: i = n \}$
\item FOR $i = n-1$ DOWNTO $1$ DO
\item \qquad $D(i) = \min\limits_{j=i+1, …, n} \{ D(j) + c(i, j) \}$ with $c(i, j) = \infty$ if $(i, j) \not\in A$
\item \qquad $NEXT(i) = argmin_{j=i+1, …, n} \{ D(j) + c(i, j) \}$
\end{enumerate}
